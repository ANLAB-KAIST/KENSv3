...
UniformDistribution::UniformDistribution(UUID seed)
...
ExpDistribution::ExpDistribution(UUID seed, Real average)
    : RandomDistribution(seed) {
...
Real ExpDistribution::nextDistribution(Real min, Real max) {
  ...
  std::uniform_real_distribution<Real> dist(exp(-((Real)1.0 / averageLocation)),
                                            1);
