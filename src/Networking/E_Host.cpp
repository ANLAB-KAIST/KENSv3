...
namespace E {
Host::Host(std::string name, size_t portNumber, NetworkSystem *system)
...
int Host::cleanUp(void) {
  std::unique_lock<std::mutex> lock(this->getSystem()->getSystemLock(),
                                    std::defer_lock);
  lock.lock();
  this->running = false;
  int missing = 0;
...
Module::Message *Host::messageReceived(Module *from, Module::Message *message) {
  ...
  if (portMessage != nullptr) {
    ...
    if (this->running == true) {
      ...
      this->sendPacketToModule(nullptr, "Ethernet",
  ...
  case PACKET_TRANSFER: {
    ...
    hostMessage->packetPass.to->packetArrived(
        fromName, std::move(hostMessage->packetPass.packet));
    break;
  }
  case SYSCALL_CALLED: {
    ...
    do {
      if (syscallIDToApplication.find(curSyscallID) ==
          syscallIDToApplication.end()) {
        syscallIDStart = curSyscallID + 1;
        syscallIDToApplication.insert(
            std::pair<UUID, Application>(curSyscallID, app));
        break;
      }
      curSyscallID++;
    ...
    *(hostMessage->syscall.returnValue) = -1;
    this->syscallIDToWakeup.insert(
        std::pair<UUID, std::pair<int *, std::condition_variable *>>(
            curSyscallID, std::pair<int *, std::condition_variable *>(
                              hostMessage->syscall.returnValue,
                              hostMessage->syscall.condVar)));

    print_log(SYSCALL_RAISED,
              "System call[syscall_no:%d, unique_id: %" PRIu64
              "] has raised from "
              "app[pid:%d] at [%s]",
              hostMessage->syscall.param.syscallNumber, curSyscallID, app->pid,
              this->getModuleName().c_str());
...
void Host::messageFinished(Module *to, Module::Message *message,
...
size_t Host::getPortCount() { return allPort.size(); }
...
void Host::sendPacket(size_t portIndex, Packet &&packet) {
  ...
  Port::Message *portMessage =
      new Port::Message(Port::PACKET_TO_PORT, std::move(packet));
  sendMessage(port, portMessage, 0);
}

Host::DefaultSystemCall::DefaultSystemCall(Host *host)
    : SystemCallInterface(0, 0, host), Module(host->getSystem()) {}
Host::DefaultSystemCall::~DefaultSystemCall() {}
Module::Message *
Host::DefaultSystemCall::messageReceived(Module *from,
...
void Host::DefaultSystemCall::messageFinished(Module *to,
                                              Module::Message *message,
                                              Module::Message *response) {
  (void)to;
  (void)response;
...
HostModule::~HostModule() { host->unregisterHostModule(name); }

std::string HostModule::getHostModuleName() { return name; }

Host *HostModule::getHost() { return host; }
void HostModule::sendPacket(std::string toModule, Packet &&packet) {
  host->sendPacketToModule(this, toModule, std::move(packet));
}
...
void Host::registerHostModule(std::string name, HostModule *hostModule) {
  this->hostModuleMap.insert(
      std::pair<std::string, HostModule *>(name, hostModule));
}
...
void Host::sendPacketToModule(HostModule *fromModule, std::string toModule,
                              Packet &&packet) {
  ...
  if (toModule.compare("Host") == 0) {
    ...
    Message *hostMessage =
        Message::make_packet_pass(fromModule, toHostModule, std::move(packet));

    this->sendMessage(this, hostMessage,
                      0); // DELAY module packet transfer delay
  }
}

void Host::registerInterface(Interface iface, Domain domain,
                             Protocol protocol) {
...
void Host::returnSystemCall(Interface iface, UUID syscallUUID, int val) {
  (void)iface;
  if (syscallIDToApplication.find(syscallUUID) ==
      syscallIDToApplication.end()) {
  ...
  SystemCallApplication::Message *appMessage =
      new SystemCallApplication::Message;
...
int Host::createFileDescriptor(Interface iface, int processID) {
  ...
  do {
    if (procInfo.fdSet.find(current) == procInfo.fdSet.end()) {
      ...
      procInfo.fdToDomain.insert(std::pair<int, Namespace>(
          current, Namespace(iface->domain, iface->protocol)));
...
void SystemCallApplication::registerApplication() {
  this->my_lock_ptr = new std::unique_lock<std::mutex>(
      this->getSystem()->getSystemLock(), std::defer_lock);
...
SystemCallApplication::SystemCallApplication(Host *host)
    : Module(host->getSystem()), NetworkLog(host->getNetworkSystem()),
      Runnable(host->getSystem(), true), my_lock_ptr(nullptr), host(host),
      pid(-1), initial_barrier_mutex()
//
{
  this->thread = nullptr;
}
...
void SystemCallApplication::initialize() {
  std::unique_lock<std::mutex> initial_barrier_lock(initial_barrier_mutex,
                                                    std::defer_lock);
...
int SystemCallApplication::E_Syscall(
  ...
  Host::Message *hostMessage =
      Host::Message::make_syscall(param, condVar, &ret);
...
Module::Message *
SystemCallApplication::messageReceived(Module *from, Module::Message *message) {
  (void)from;
...
void SystemCallApplication::messageFinished(Module *to,
                                            Module::Message *message,
                                            Module::Message *response) {
  (void)to;
  assert(response == nullptr);
...
void SystemCallApplication::messageCancelled(Module *to,
                                             Module::Message *message) {
  (void)to;
  (void)message;
  assert(0);
