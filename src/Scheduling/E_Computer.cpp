...
Module::Message *Computer::messageReceived(Module *from,
  ...
  case JOB_RUN: {
    ...
    print_log(Log::DEBUG,
              "Job run[%" PRIu64 "] %" PRIu64 " %" PRIu64 " %" PRIu64 "",
              this->getSystem()->getCurrentTime(), runningJob->raisedAt,
              runningJob->executionTime, runningJob->deadLine);
    Processor *curProc = dynamic_cast<Processor *>(from);
    assert(curProc != nullptr);

    curProc->isRunning = false;
    curProc->deleteJob();
    this->scheduler->jobFinished(this, curProc, runningJob);
  ...
  case JOB_CHECK: {
    ...
    print_log(Log::DEBUG,
              "Job check[%" PRIu64 "] %" PRIu64 " %" PRIu64 " %" PRIu64 "",
              this->getSystem()->getCurrentTime(), checkingJob->raisedAt,
              checkingJob->executionTime, checkingJob->deadLine);
    assert(checkingJob != nullptr);

    checkingJob->isChecking = false;
    this->scheduler->jobFinished(this, checkingJob->processor, checkingJob);
...
void Computer::messageFinished(Module *to, Module::Message *message,
...
Computer::Computer(System *system, CPUID numCPU, Scheduler *scheduler,
                   Time overhead)
    : Module(system), Log() {
...
void Computer::raiseJob(Task *task, Time executionTime, Time deadline) {
  ...
  Job *job = new Job(this->getSystem()->getCurrentTime(), executionTime,
                     deadline, task);
  ...
  this->scheduler->jobRaised(this, job);
}
...
CPUID Computer::getNumCPU() { return this->cpuVector.size(); }
size_t Computer::getDone() { return this->done; }
size_t Computer::getMiss() { return this->miss; }
size_t Computer::getRaised() { return this->raised; }

Processor *Computer::getCPU(CPUID cpuID) { return this->cpuVector[cpuID]; }
