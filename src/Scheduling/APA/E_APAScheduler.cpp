...
APAWeakScheduler::~APAWeakScheduler() { this->jobQueue.clear(); }

void APAWeakScheduler::jobFinished(Computer *computer, Processor *processor,
                                   Job *job) {
  (void)processor;
...
void APAWeakScheduler::jobRaised(Computer *computer, Job *job) {
  assert(computer);
  assert(job);
...
APAStrongScheduler::~APAStrongScheduler() { jobQueue.clear(); }

void APAStrongScheduler::jobFinished(Computer *computer, Processor *processor,
                                     Job *job) {
  (void)processor;
...
void APAStrongScheduler::jobRaised(Computer *computer, Job *job) {
  assert(computer);
  assert(job);
...
std::list<void *> APAStrongScheduler::BFS(Computer *computer, Job *job,
                                          CPUID targetCPU) {
  ...
  while (!queue.empty()) // while Q is not empty loop
  {
    ...
    if (currentPair.first == true) {
      ...
      for (CPUID adjacentCPU :
           curTask->getAffinity()) // u ← G.adjacentVertex(t,e)
      {
        ...
        if (visited.find(nextCPU) == visited.end()) // if u is not in V then
        {
          ...
          queue.push(
              std::pair<bool, void *>(false, nextCPU)); // enqueue u onto Q
...
bool APAStrongScheduler::scheduleSingle(Computer *computer, Job *job) {
  ...
  bool found = false;
  for (CPUID candidateProcessorID = 0;
       candidateProcessorID < computer->getNumCPU(); candidateProcessorID++) {
    ...
    if (currentJob == nullptr) {
      ...
      found = true;
      break;
    } else if (compare(job, currentJob)) // Job to be scheduled can evict the
                                         // end of the chain
    {
      if (!found) {
        candidateCPU = lastProcessor;
  ...
  if (found) {
    ...
    while (candidateChain.size() > 0) {
      ...
      Processor *movingProcessor =
          static_cast<Processor *>(candidateChain.front());
      candidateChain.pop_front();
