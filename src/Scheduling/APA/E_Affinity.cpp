...
AffinityTask::AffinityTask(Affinity affinity, Computer *computer, Time period,
                           Time executionTime, Time startOffset)
...
AffinityTask::AffinityTask(const AffinityTask &orig, Computer *computer)
    : SporadicTask(computer, orig.minPeriod, orig.worstExecution,
                   orig.startOffset) {
...
bool GraphNode::isTask() const { return this->type; }

GraphNode::~GraphNode() {}

void AffinityTask::DFS(
    std::list<std::list<GraphNode>> &saveAt,
    const std::unordered_map<CPUID, TaskSet> &cpuToTaskList,
    const std::unordered_map<AffinityTask *, Affinity> &taskToCPUList,
    const GraphNode &start, const GraphNode &target,
    const std::list<GraphNode> &visited) {
  ...
  if (start.isTask()) {
    ...
    for (CPUID adjacentCPU : iter->second) // u ← G.adjacentVertex(t,e)
    {
      ...
      if (ifVisited)
        continue;
      DFS(saveAt, cpuToTaskList, taskToCPUList, adjacentCPU, target,
          currentVisited);
    }
  } else {
    ...
    for (AffinityTask *adjacentTask : iter->second) // u ← G.adjacentVertex(t,e)
    {
      ...
      if (ifVisited)
        continue;
      DFS(saveAt, cpuToTaskList, taskToCPUList, adjacentTask, target,
          currentVisited);
    }
  }
}

std::list<std::list<GraphNode>>
AffinityTask::allPath(const TaskSet &taskSet, const GraphNode &start,
                      const GraphNode &target, const Affinity &excludeID,
                      const TaskSet &excludeTask) {
  ...
  for (auto task : taskSet) {
    ...
    taskToCPUList.insert(
        std::pair<AffinityTask *, Affinity>(task, affinityCopy));
  ...
  DFS(returnValue, cpuToTaskList, taskToCPUList, start, target,
      std::list<GraphNode>());

  return returnValue;
}

std::list<GraphNode> AffinityTask::BFS(const TaskSet &taskSet,
                                       const GraphNode &start,
                                       const GraphNode &target,
                                       const Affinity &excludeID,
                                       const TaskSet &excludeTask) {
  ...
  std::queue<GraphNode>
      queue; // create a queue Q, true is Job, false is processor
  ...
  while (!queue.empty()) // while Q is not empty loop
  {
    ...
    if (currentItem.isTask()) {
      if (target.isTask())
        if (target.getTask() ==
            currentItem.getTask()) // if t is what we are looking for then
        {
          // return t
          reachable = true;
          break;
      ...
      if (!target.isTask())
        if (target.getCPUID() ==
            currentItem.getCPUID()) // if t is what we are looking for then
        {
          // return t
          reachable = true;
          break;
    ...
    if (currentItem.isTask()) {
      ...
      for (CPUID adjacentCPU :
           taskToCPUList.find(curTask)->second) // u ← G.adjacentVertex(t,e)
      {
        if (visitedCPU.find(adjacentCPU) ==
            visitedCPU.end()) // if u is not in V then
        {
          ...
          cpuToPrevTask.insert(
              std::pair<CPUID, AffinityTask *>(adjacentCPU, curTask));
        }
      }
    } else {
      ...
      for (AffinityTask *adjacentTask :
           iter->second) // u ← G.adjacentVertex(t,e)
      {
        if (visitedTask.find(adjacentTask) == visitedTask.end()) {
          ...
          taskToPrevCPU.insert(
              std::pair<AffinityTask *, CPUID>(adjacentTask, curCPU));
        }
      }
    }
  }
...
bool AffinityTask::staticWeakAnalysis(const TaskSet &taskSet, Time overhead) {
  ...
  for (AffinityTask *task : taskSet)
    responseTime.insert(
        std::pair<AffinityTask *, Time>(task, task->worstExecution));
    ...
    for (auto current : responseTime) {
      ...
      for (Affinity s : powerSet) {
        ...
        for (CPUID selectedCPU : s) {
          for (AffinityTask *highPriorityTask : taskSet) {
            ...
            if (highPriorityTask == curTask)
              continue;
            if (highPriorityTask->affinity.find(selectedCPU) ==
                highPriorityTask->affinity.end())
              continue;
        ...
        for (AffinityTask *highPriorityTask : highPrioritySet) {
          Time interferenceCount =
              currentResponse / highPriorityTask->minPeriod;
          Time remaining = currentResponse % highPriorityTask->minPeriod;
          Time interference =
              interferenceCount * highPriorityTask->worstExecution +
      ...
      newResponseTime.insert(
          std::pair<AffinityTask *, Time>(curTask, nextResponse));
  ...
  for (auto iter : responseTime) {
    if (iter.second > iter.first->minPeriod) {
      possible = false;
      iter.first->print_log(WARN,
                            "Execution time: %" PRIu64 ", Period: %" PRIu64
                            ", Response time: %" PRIu64 "",
                            iter.first->worstExecution, iter.first->minPeriod,
                            iter.second);
    } else {
      iter.first->print_log(INFO,
                            "Execution time: %" PRIu64 ", Period: %" PRIu64
                            ", Response time: %" PRIu64 "",
                            iter.first->worstExecution, iter.first->minPeriod,
                            iter.second);
...
void AffinityTask::combinePossibleTaskSet_recursive(
    const std::unordered_map<CPUID, std::list<TaskSet>> &possibleReplacement,
    const std::unordered_map<CPUID, CPUID> &nextCPU, const CPUID &currentCPU,
    const std::list<TaskSet> &visited, std::list<TaskSet> &saveAt) {
  ...
  if (nextCPUIter == nextCPU.end()) {
    ...
    for (auto currentItem : currentList->second) {
      ...
      combinePossibleTaskSet_recursive(possibleReplacement, nextCPU, nextCPUID,
                                       newVisited, saveAt);
    }
  }
}

std::list<AffinityTask::TaskSet> AffinityTask::combinePossibleTaskSet(
    const std::unordered_map<CPUID, std::list<TaskSet>> &possibleReplacement) {
  ...
  combinePossibleTaskSet_recursive(possibleReplacement, nextCPUIDMap, firstCPU,
                                   std::list<TaskSet>(), returnValue);
  ...
  while (true) {
    ...
    for (auto current : responseTime) {
      ...
      for (Affinity s : powerSet) {
        ...
        for (auto cpu : s) {
          possibleReplacement.insert(
              std::pair<CPUID, std::list<TaskSet>>(cpu, std::list<TaskSet>()));
        }
        ...
        for (CPUID selectedCPU : s) {
          ...
          for (auto alternative : allCPU) {
            if (ignoreCPU.find(alternative) != ignoreCPU.end())
              continue;
            auto allPaths = allPath(taskSet, selectedCPU, alternative,
                                    ignoreCPU, ignoreTask);
            for (auto path : allPaths) {
              if (path.size() > 0) {
                ...
                possibleReplacement.find(selectedCPU)
                    ->second.push_back(highTaskSet);
              }
            }
          }
        }
...
AffinityTask::TaskSet
AffinityTask::copyTaskSet(Computer *computer,
                          const AffinityTask::TaskSet &orig) {
...
AffinityTask::TaskSet AffinityTask::generateTaskSet(
    Computer *computer, Size numTask, RandomDistribution *affinityDistribution,
  ...
  for (Real util : utilDistribution->distribute(numTask, targetUtilization)) {
    ...
    Real realPeriod =
        periodDistribution->nextDistribution(minPeriod, maxPeriod);
    ...
    AffinityTask *task =
        new AffinityTask(affinity, computer, period, execution, 0);
    ret.insert(task);
  }
