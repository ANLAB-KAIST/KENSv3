 ...
 * However, it may be extended to picosecond resolution or real number
 * resolution. TimerUtil provides consistent view of Time.
 */
class TimeUtil {
private:
  TimeUtil();
...
public:
   ...
   * For example, multiplier for millisecond is 1,000,000 and for microsecond is
   * 1,000.
   *
   * @param unit TimeUtil::TimeUnit enumeration
   *
  ...
  /**
   * @brief Converts nanosecond time value to a time in given time unit in
   * string.
   *
   * @param time Time value in nanosecond
   * @param unit TimeUtil::TimeUnit enumeration
